...
export async function POST() {
  ...
  if (db.tasks.length === 0) {
    ...
    seed.forEach((t, i) =>
      store.addTask({ title: t.title, description: t.description, contributorAddress: contributors[i % contributors.length]!, amountUsdc: perTask > 0 ? perTask : t.amountUsdc, status: t.status, evidence: t.evidence }),
    );
  } else {
